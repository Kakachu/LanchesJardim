...
namespace LanchesJardim.Areas.Admin.Controllers
{
    ...
    public class AdminLanchesController : Controller
    {
        ...
        {
            var lanche = await _context.Lanches.FindAsync(id);
            if (CarrinhoLancheExists(id))
            {
                var ItensCarrinho = _context.CarrinhoCompraItems.Where(e => e.Lanche.LancheId == id).ToList();
                _context.CarrinhoCompraItems.RemoveRange(ItensCarrinho);
            }
        ...
        private bool CarrinhoLancheExists(int LancheId)
        {
            return _context.CarrinhoCompraItems.Any(e => e.Lanche.LancheId == LancheId);
        }
    }
}
